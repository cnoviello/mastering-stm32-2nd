...
class STM32Flasher(object):
    def __init__(self, serialPort, baudrate=38400):
        self.serial = serial.Serial(serialPort, baudrate=baudrate, timeout=15)
...
if __name__ == '__main__':
    ...
    for e in flasher.writeImage(sys.argv[2]):
        ...
        if "loc" in e:
            ...
            print("\r\tWriting address: %s --- %s" % (hex(e["loc"]), "CRC ERROR: RESENDING" if e["resend"]  else "CRC OK"), end=end)
            sys.stdout.flush()
