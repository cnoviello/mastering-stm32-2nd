...
#define APP_START_ADDRESS 0x0800A000 /* In STM32G474RE this corresponds with the start
                                        address of Page 19 */
#define APP_START_PAGE	  0x13

#define SRAM_SIZE         128*1024     // STM32G474RE has 128KB of RAM
#define SRAM_END          (SRAM_BASE + SRAM_SIZE)
#define FLASH_TOTAL_PAGES 127

#define ENABLE_BOOTLOADER_PROTECTION 0
#define FIRST_PAGES_TO_PROTECT 0x0
#define LAST_PAGES_TO_PROTECT  0x12
...
void cmdErase(uint8_t *pucData) {
  ...
  if (ulCrc == HAL_CRC_Calculate(&hcrc, pulCmd, 2) &&
    ...
    eraseInfo.NbPages = pucData[1] == 0xFF ? FLASH_TOTAL_PAGES - 20 : pucData[1];
...
void CHECK_AND_SET_FLASH_PROTECTION(void) {
  ...
  if (obConfig.WRPStartOffset > FIRST_PAGES_TO_PROTECT) {
	obConfig.WRPStartOffset = FIRST_PAGES_TO_PROTECT;
	obConfig.WRPEndOffset = LAST_PAGES_TO_PROTECT;
